...
<?php
	...
	class class_model{
		...
		public function login_admin_aide($username, $password, $status){
			$stmt = $this->conn->prepare("SELECT * FROM `tbl_administrative_aide` WHERE `username` = ? AND `password` = ? AND `status` = ?") or die($this->conn->error);
			...
			if($stmt->execute()){
				...
				return array(
					'admin_aide_id'=> htmlentities($fetch['admin_aide_id']),
					'count'=>$valid
				);
			}
		}
 
		public function admin_aide_account($admin_aide_id){
			$stmt = $this->conn->prepare("SELECT * FROM `tbl_administrative_aide` WHERE `admin_aide_id` = ?") or die($this->conn->error);
		    $stmt->bind_param("i", $admin_aide_id);
			if($stmt->execute()){
				$result = $stmt->get_result();
				$fetch = $result->fetch_array();
				return array(
    ...
    public function fetchAll_payment(){ 
	    ...
	    public function add_user($complete_name, $desgination, $email_address, $phone_number, $username, $password, $status){
	       $stmt = $this->conn->prepare("INSERT INTO `tbl_administrative_aide` (`complete_name`, `desgination`, `email_address`, `phone_number`, `username`, `password`, `status`) VALUES(?, ?, ?, ?, ?, ?, ?)") or die($this->conn->error);
	    ...
	    public function fetchAll_user(){ 
            $sql = "SELECT * FROM  tbl_administrative_aide";
				$stmt = $this->conn->prepare($sql); 
				$stmt->execute();
				$result = $stmt->get_result();
		        $data = array();
	    ...
	    public function edit_user($complete_name, $desgination, $email_address, $phone_number, $username, $password, $status, $admin_aide_id){
			$sql = "UPDATE `tbl_usermanagement` SET  `complete_name` = ?, `desgination` = ?, `email_address` = ?, `phone_number` = ?, `username` = ?, `password` = ?, `status` = ?  WHERE admin_aide_id = ?";
			 $stmt = $this->conn->prepare($sql);
			$stmt->bind_param("sssssssi", $complete_name, $desgination, $email_address, $phone_number, $username, $password, $status, $admin_aide_id);
			if($stmt->execute()){
				$stmt->close();
				$this->conn->close();
				return true;
	   ...
	   public function delete_user($admin_aide_id){
				$sql = "DELETE FROM tbl_usermanagement WHERE admin_aide_id = ?";
				 $stmt = $this->conn->prepare($sql);
				$stmt->bind_param("i", $admin_aide_id);
				if($stmt->execute()){
					$stmt->close();

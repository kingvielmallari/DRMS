...
<?php
	...
	class class_model{
		...
		public function login_admin_assist($username, $password, $status){
			$stmt = $this->conn->prepare("SELECT * FROM `tbl_administrative_assistant` WHERE `username` = ? AND `password` = ? AND `status` = ?") or die($this->conn->error);
			...
			if($stmt->execute()){
				...
				return array(
					'admin_assist_id'=> htmlentities($fetch['admin_assist_id']),
					'count'=>$valid
				);
			}
		}
 
		public function admin_assist_account($admin_assist_id){
			$stmt = $this->conn->prepare("SELECT * FROM `tbl_administrative_assistant` WHERE `admin_assist_id` = ?") or die($this->conn->error);
		    $stmt->bind_param("i", $admin_assist_id);
			if($stmt->execute()){
				$result = $stmt->get_result();
				$fetch = $result->fetch_array();
				return array(
    ...
    public function fetchAll_payment(){ 
	    ...
	    public function add_user($complete_name, $desgination, $email_address, $phone_number, $username, $password, $status){
	       $stmt = $this->conn->prepare("INSERT INTO `tbl_administrative_assistant` (`complete_name`, `desgination`, `email_address`, `phone_number`, `username`, `password`, `status`) VALUES(?, ?, ?, ?, ?, ?, ?)") or die($this->conn->error);
	    ...
	    public function fetchAll_user(){ 
            $sql = "SELECT * FROM  tbl_administrative_assistant";
				$stmt = $this->conn->prepare($sql); 
				$stmt->execute();
				$result = $stmt->get_result();
		        $data = array();
	    ...
	    public function edit_user($complete_name, $desgination, $email_address, $phone_number, $username, $password, $status, $admin_assist_id){
			$sql = "UPDATE `tbl_administrative_assistant` SET  `complete_name` = ?, `desgination` = ?, `email_address` = ?, `phone_number` = ?, `username` = ?, `password` = ?, `status` = ?  WHERE admin_assist_id = ?";
			 $stmt = $this->conn->prepare($sql);
			$stmt->bind_param("sssssssi", $complete_name, $desgination, $email_address, $phone_number, $username, $password, $status, $admin_assist_id);
			if($stmt->execute()){
				$stmt->close();
				$this->conn->close();
				return true;
	   ...
	   public function delete_user($admin_assist_id){
				$sql = "DELETE FROM tbl_administrative_assistant WHERE admin_assist_id = ?";
				 $stmt = $this->conn->prepare($sql);
				$stmt->bind_param("i", $admin_assist_id);
				if($stmt->execute()){
					$stmt->close();
